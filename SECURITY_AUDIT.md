...
Release validation on the isolated maintenance branch now passes with Next.js `15.5.21`: frozen pnpm install, TypeScript typecheck, all 70 unit tests, optimized production build, dependency audit at moderate-or-higher severity, CodeRabbit review, and the normal Vercel preview all succeed.

The PR should remain draft until the production read-only Playwright smoke test and explicit cross-client authorization regression coverage are run with the required credentials/environment.
...
Final validation used Node 24 and pnpm 11.7.0 with the committed lockfile:
...
The initial audit after the Next.js patch identified five transitive advisories in `sharp`, `nanoid`, and `postcss`. The branch now pins patched transitive versions through `pnpm-workspace.yaml` overrides:

- `sharp 0.35.3`
- `nanoid 3.3.18`
- `postcss 8.5.26`

The lockfile was regenerated by pnpm in GitHub Actions, then the final CI workflow was restored to read-only permissions and frozen-lockfile installation before the passing validation run.
...
GitHub's Vercel integration successfully builds the Bridget Pope Designs preview with the final frozen lockfile and patched dependencies. The connected Vercel API still returns 404 for the Bridget project, so production environment variables, runtime logs, domain configuration, and Vercel security settings remain unavailable through the connector. Production-level audit remains incomplete until that project is exposed to the connected Vercel account/API.