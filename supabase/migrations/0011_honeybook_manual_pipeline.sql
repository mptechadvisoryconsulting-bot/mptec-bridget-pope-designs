-- HoneyBook-ready manual CRM pipeline.
-- Additive only: in-app invoices/proposals/payments remain the financial authority.
-- HoneyBook references are optional external links for manual workflow.
...
-- support structured automation payloads (pipeline + HoneyBook inbound events)
alter table bpd_automation_logs
  add column if not exists metadata jsonb not null default '{}'::jsonb;

-- project pipeline stage for manual HoneyBook-ready workflow
alter table bpd_projects
  add column if not exists pipeline_stage text not null default 'lead_received';

alter table bpd_projects
  add column if not exists honeybook_url text;
...
create table if not exists bpd_pipeline_events (
  id uuid primary key default gen_random_uuid(),
  project_id uuid references bpd_projects(id) on delete cascade,
  lead_id uuid references bpd_leads(id) on delete set null,
  stage text not null,
  source text not null default 'manual' check (source in ('manual','honeybook_api','system')),
  actor_id uuid references bpd_profiles(id) on delete set null,
...
create index if not exists bpd_idx_pipeline_events_project
  on bpd_pipeline_events(project_id, created_at desc);

create index if not exists bpd_idx_pipeline_events_lead
  on bpd_pipeline_events(lead_id)
  where lead_id is not null;

create index if not exists bpd_idx_pipeline_events_stage
  on bpd_pipeline_events(stage);

create index if not exists bpd_idx_projects_pipeline_stage
  on bpd_projects(pipeline_stage);

alter table bpd_pipeline_events enable row level security;

drop policy if exists "Admins can manage bpd_pipeline_events" on bpd_pipeline_events;
create policy "Admins can manage bpd_pipeline_events"
on bpd_pipeline_events
for all
to authenticated
using (bpd_is_admin())
with check (bpd_is_admin());
